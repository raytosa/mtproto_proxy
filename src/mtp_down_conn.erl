...
send(Conn, Data) ->
  ...
  %%up  io:format("mtp_down_conn      send --- ~p ~n",[byte_size(Data)]),
...
down_send(Packet, #state{sock = Sock, codec = Codec, dc_id = DcId} = St) ->
    ...
    mtp_metric:rt(
      ...
      fun() ->
         ...
         io:format("mtp_down_conn      down_send ~p  ~n",  [Sock]),%%io:format("mtp_down_conn      down_send ---~p~n  ~p  ~n",  [byte_size(Encoded),Encoded]),
